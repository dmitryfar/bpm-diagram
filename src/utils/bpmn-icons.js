...
export function _drawUserTaskIcon(paper, startX, startY, element)
{

  var color = bpmnDrawHelper._bpmnGetColor(element, "#d1b575");
	var path = paper.path("m 1,17 16,0 0,-1.7778 -5.333332,-3.5555 0,-1.7778 c 1.244444,0 1.244444,-2.3111 1.244444,-2.3111 l 0,-3.0222 C 12.555557,0.8221 9.0000001,1.0001 9.0000001,1.0001 c 0,0 -3.5555556,-0.178 -3.9111111,3.5555 l 0,3.0222 c 0,0 0,2.3111 1.2444443,2.3111 l 0,1.7778 L 1,15.2222 1,17 17,17");
	path.attr({
		"opacity": 1,
		"stroke": "none",
		"fill": color
...
export function _drawServiceTaskIcon(paper, startX, startY, element)
{
  ...
  path.attr({
	...
	serviceTaskIcon.push(path);

	serviceTaskIcon.transform("T" + startX + "," + startY);
  return path
}

export function _drawScriptTaskIcon(paper, startX, startY, element)
{
  // _drawSendTaskIcon(paper, startX, startY, element)
  // return
  var color = bpmnDrawHelper._bpmnGetColor(element, "#72a7d0");
	var path = paper.path("m 5,2 0,0.094 c 0.23706,0.064 0.53189,0.1645 0.8125,0.375 0.5582,0.4186 1.05109,1.228 1.15625,2.5312 l 8.03125,0 1,0 1,0 c 0,-3 -2,-3 -2,-3 l -10,0 z M 4,3 4,13 2,13 c 0,3 2,3 2,3 l 9,0 c 0,0 2,0 2,-3 L 15,6 6,6 6,5.5 C 6,4.1111 5.5595,3.529 5.1875,3.25 4.8155,2.971 4.5,3 4.5,3 L 4,3 z");
	path.attr({
		"opacity": 1,
		"stroke": "none",
		"fill": color
	...
	scriptTaskIcon.push(path);

	scriptTaskIcon.transform("T" + startX + "," + startY);
  return path
}

export function _drawBusinessRuleTaskIcon(paper, startX, startY, element)
{
  var color = bpmnDrawHelper._bpmnGetColor(element, "#B3B1B3");

  var path = paper.path("m 1,2 0,14 16,0 0,-14 z m 1.45458,5.6000386 2.90906,0 0,2.7999224 -2.90906,0 z m 4.36364,0 8.72718,0 0,2.7999224 -8.72718,0 z m -4.36364,4.1998844 2.90906,0 0,2.800116 -2.90906,0 z m 4.36364,0 8.72718,0 0,2.800116 -8.72718,0 z");
  ...
  businessRuleTaskIcon.push(path);

  businessRuleTaskIcon.transform("T" + startX + "," + startY);
  return path
}

export function _drawSendTaskIcon(paper, startX, startY, element)
{
  var color = bpmnDrawHelper._bpmnGetColor(element, "#16964d");
  startX = startX + 0.5
  startY = startY + 0.5
  ...
  var pathWidth = path.getBBox().width;
  var pathHeight = path.getBBox().height;

  var rect = paper.rect(0, 0 + 2, pathWidth + 2, pathHeight + 2);
  rect.attr({
    "stroke": color,
    "fill": "none"
  });
	...
	sendTaskIcon.push(rect, path);

	sendTaskIcon.transform("T" + startX + "," + startY);
  return [path, rect]
}
export function _drawSendTaskIcon2(paper, startX, startY, element)
{
  var color = bpmnDrawHelper._bpmnGetColor(element, "#16964d");
...
export function _drawManualTaskIcon(paper, startX, startY, element)
{
  var color = bpmnDrawHelper._bpmnGetColor(element, "#d1b575");
	...
	manualTaskIcon.push(path);

	manualTaskIcon.transform("T" + startX + "," + startY);
  return path
}

export function _drawReceiveTaskIcon(paper, startX, startY, element)
{
  var color = bpmnDrawHelper._bpmnGetColor(element, "#16964d");
...
export function _drawCamelTaskIcon(paper, startX, startY)
{
...
export function _drawHttpTaskIcon(paper, startX, startY)
{
...
export function _drawShellTaskIcon(paper, startX, startY)
{
...
export function _drawDecisionTaskIcon(paper, startX, startY)
{
...
export function _drawMuleTaskIcon(paper, startX, startY)
{
...
export function _drawEventIcon(paper, element)
{
	if (element.eventDefinition && element.eventDefinition.type)
	{
		if ("timer" === element.eventDefinition.type)
		{
			return _drawTimerIcon(paper, element);
		}
		else if ("error" === element.eventDefinition.type)
		{
      return _drawErrorIcon(paper, element);
		}
		else if ("signal" === element.eventDefinition.type)
		{
      return _drawSignalIcon(paper, element);
		}
		else if ("message" === element.eventDefinition.type)
		{
      return _drawMessageIcon(paper, element);
		}
		else if ("compensate" === element.eventDefinition.type)
		{
      return _drawCompensateIcon(paper, element);
		}
	}
  return null
}

export function _drawTimerIcon(paper, element)
{
	var x = element.x + (element.width / 2);
	var y = element.y + (element.height / 2);

	var strokeColor = bpmnDrawHelper._bpmnGetColor(element, conf.MAIN_STROKE_COLOR);

	let timerCircleSize = 10;
	var circle = paper.circle(x, y, timerCircleSize);
  ...
  var pathWidth = path.getBBox().width;
  var pathHeight = path.getBBox().height;
  var x = element.x + element.width/2 - pathWidth/2 ;
  var y = element.y + element.height/2 - pathHeight/2;

  // paper.rect(element.x, element.y, element.width, element.height).attr({"stroke": "violet", "stroke-width": 1,});
  // paper.rect(x, y, pathWidth, pathHeight).attr({"stroke": "skyblue", "stroke-width": 1,});
...
export function _drawErrorIcon(paper, element)
{
  ...
  path.transform("T" + (x - 9.3386512) + "," + (y - 6.8323057));
	return path;
}

export function _drawSignalIcon(paper, element)
{
  var strokeColor = bpmnDrawHelper._bpmnGetColor(element, conf.MAIN_STROKE_COLOR);
  ...
  // paper.rect(x, y, pathWidth, pathHeight).attr({"stroke": "skyblue", "stroke-width": 1,});
	...
	path.transform("T" + (x - 8.7124971) + "," + (y - 8.5759512 - 1));
	return path;
}

export function _drawMessageIcon(paper, element)
{
  var strokeColor = bpmnDrawHelper._bpmnGetColor(element, conf.MAIN_STROKE_COLOR);
  ...
  // paper.rect(x, y, pathWidth, pathHeight).attr({"stroke": "skyblue", "stroke-width": 1,});
	...
	path.transform("T" + (x - 1) + "," + (y - 3));
	return path;
}

export function _drawCompensateIcon(paper, element)
{
  var strokeColor = bpmnDrawHelper._bpmnGetColor(element, conf.MAIN_STROKE_COLOR);
  ...
  var path = paper.path("M 16,14 L 24,21 L 24,8 z M 24,14 L 32,21 L 32,8 z");
  path.attr({
    "stroke": strokeColor,
    "stroke-width": 1,
    "fill": fill
